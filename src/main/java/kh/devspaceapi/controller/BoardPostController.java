...
public class BoardPostController {
	...
	/**
	 * 게시글 생성 - Body로 title, category, content, (선택) user.userId 전달 - 201 Created 반환
	 * (바디는 없음)
	 */
	@PostMapping
	public ResponseEntity<Void> create(@RequestBody BoardPost body,
			@AuthenticationPrincipal CustomUserDetails principal) throws Exception {
		boardPostService.insertByUser(body, principal.getUsername());
		return ResponseEntity.status(HttpStatus.CREATED).build();
	}

	/**
	 * 게시글 상세 조회 - 조회수 로그 1건 적재 + 합계(viewCount/commentCount) 계산하여 DTO로 반환
	 */
	@GetMapping("/{id}")
	...
	/**
	 * 게시글 검색 - searchType: title | nickname | title+content - keyword: 검색어,
	 * category: 카테고리 필터 - 페이지네이션/정렬 동일
	 */
	@GetMapping("/search")
	...
	/**
	 * 엔티티 -> DTO 변환 헬퍼 - viewCount / commentCount 는 엔티티 필드가 아니라, PostViewLog 합계로
	 * 계산한 값
	 */
	...
	/**
	 * 댓글 목록 (페이지네이션) - 기본값: curPage=0, pageSize=10 - 최신 순(createdAt DESC) -
	 * active=true 인 댓글만 노출(서비스/리포지토리) - 예:
	 * /api/board-posts/1/comments?curPage=0&pageSize=10
	 */
	...
	public Page<PostCommentResponseDto> listComments(@PathVariable Long boardPostId,
		...
		Pageable pageable = PageRequest.of(Math.max(curPage, 0), Math.max(pageSize, 1),
				Sort.by(Sort.Direction.DESC, "createdAt"));
		return postCommentService.page(boardPostId, TargetType.BOARD, pageable);
	}

	/**
	 * 댓글 작성 - Body: { "content": "내용" } - userId는 지금 null로 넣어 테스트(인증 붙이면
	 * SecurityContext에서 가져와 세팅 가능) - 생성 시 active=true(보임)로 저장되도록
