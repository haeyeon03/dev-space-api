 ...
 * 자유게시판 서비스 구현.
 *
 * 보이는 글: active=true 숨김(삭제) 처리: active=false (소프트 삭제) 상세 조회 시(Post 상세) 조회수는
 * PostViewLog 테이블에 1줄 적재(viewCount=1) 댓글 수 증감도 PostViewLog에 합산 방식으로
 * 관리(commentCount 누적) 목록/검색은 Repository에서 active=true만 조회
 */
@Service
...
public class BoardPostServiceImpl implements BoardPostService {
	...
	/**
	 * 게시글 생성. 생성/수정 시각 세팅 후 저장 BaseEntity의 @PrePersist도 동작(활성화 기본 true 등)
	 */
	@Override
	@Transactional
	...
	/**
	 * 게시글 단건 조회(소유권/활성여부 체크는 정책에 따라 컨트롤러/서비스에서 별도 처리 가능). 존재 여부만 체크하여 반환
	 */
	@Override
	@Transactional
	...
	/**
	 * 게시글 수정. 제목/내용/카테고리 변경 + updatedAt 갱신
	 */
	@Override
	@Transactional
	...
	/**
	 * 게시글 삭제(소프트 삭제). active=false 로 전환, updatedAt 갱신
	 */
	@Override
	@Transactional
	...
	/**
	 * 게시글 목록(보이는 글만). 정렬/페이징은 Pageable에 위임
	 */
	@Override
	...
	/**
	 * 검색(보이는 글만). type/title/nickname/title+content 조합 처리 카테고리 필터 유/무에 따라 분기
	 */
	@Override
	...
	/**
	 * 게시글 상세 조회(+조회수 1 적재). PostViewLog에 한 줄 insert(viewCount=1) -> 합계는 쿼리로 계산 합계
	 * 조회 결과를 DTO로 반환
	 */
	@Override
	@Transactional
	public BoardPostResponseDto getPost(Long postId) {
		...
		postViewLogRepository.save(PostViewLog.builder().targetId(post.getBoardPostId()).targetType(TargetType.BOARD)
				.userId(null).viewDate(new Timestamp(System.currentTimeMillis())).viewCount(1) // 조회수 +1 의미
	...
	/**
	 * 댓글 수 +1 기록. 실제 댓글 저장/삭제는 PostCommentService에서 수행 여기서는 합계 집계를 위해 로그만 쌓음
	 * API(필요 시 사용)
	 */
	@Override
	@Transactional
	public void plusCommentCount(Long postId) {
		postViewLogRepository.save(PostViewLog.builder().targetId(postId).targetType(TargetType.BOARD).userId(null)
				.viewDate(new Timestamp(System.currentTimeMillis())).viewCount(0).commentCount(1) // 댓글수 +1
				.build());
	}
	...
	public int getViewCountOf(Long postId) {
		Long sum = em
				.createQuery("select coalesce(sum(p.viewCount),0) from PostViewLog p "
	...
	/**
	 * 댓글 수 합계 조회(PostViewLog 기반). 댓글 작성 시 +1, 삭제 시 -1 로 누적되어 합계가 계산됨.
	 */
	@Override
	...
	public void insertByUser(BoardPost body, String userIdFromToken) throws Exception {
		Users user = usersRepository.findByUserId(userIdFromToken)
				.orElseThrow(() -> new IllegalStateException("User not found: " + userIdFromToken));
		body.setUser(user);
		boardPostRepository.save(body);
	}
}
